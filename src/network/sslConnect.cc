...
SSLConnection::SSLConnection(string ip, int port, int type) {
    ...
    switch (type) {
        case IN_SERVERSIDE:
            ...
            if (setsockopt(listenFd_, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int)) < 0) {
                fprintf(stderr, "SSLConnection: cannot set the port reusable.\n");
                exit(EXIT_FAILURE);
            }
            if (bind(listenFd_, (struct sockaddr*)&socketAddr_, sizeof(socketAddr_)) == -1) {
                fprintf(stderr, "SSLConnection: cannot not bind to socketFd\n"
                                "\tMay cause by shutdown server before client\n"
                                "\tWait for 1 min and try again\n");
                fprintf(stderr, "SSLConnection: %s\n", strerror(errno));
                exit(EXIT_FAILURE);
            }
        ...
        case IN_CLIENTSIDE:
            ...
            break;
        default:
            fprintf(stderr, "SSLConnection: error connection type.\n");
            exit(EXIT_FAILURE);
    }
    ...
    switch (type) {
        case IN_SERVERSIDE:
            fprintf(stderr, "SSLConnection: Init the connection to port %d\n", port_);
            break;
        case IN_CLIENTSIDE:
            fprintf(stderr, "SSLConnection: Init the connection to <%s:%d>\n", serverIP_.c_str(), port_);
            break; 
        default:
            fprintf(stderr, "SSLConnection: error connection type.\n");
            exit(EXIT_FAILURE);
...
SSLConnection::~SSLConnection() {
    ...
    close(listenFd_);
    fprintf(stderr, "SSLConnection: Destory the SSLConnection\n");
}
...
void SSLConnection::Finish(pair<int, SSL*> sslPair) {
    ...
    if (ret != 0) {
        fprintf(stderr, "SSLConnection: first shutdown the socket in client side error, "
                "ret: %d\n", ret);
        exit(EXIT_FAILURE);
    }
    ...
    if (tmp != 1) {
        fprintf(stderr, "SSLConnection: shutdown the ssl socket fail, ret: %d\n", tmp);
        exit(EXIT_FAILURE);
    }

    fprintf(stderr, "SSLConnection: shutdown the SSL connection successfully.\n");
...
pair<int, SSL*> SSLConnection::ConnectSSL() {
    ...
    if (connect(socketFd, (struct sockaddr*)&socketAddr_, sizeof(socketAddr_)) < 0) {
        fprintf(stderr, "SSLConnection: cannot connect on the socket: %s.\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
    ...
    if (SSL_connect(sslConnectionPtr) != 1) {
        fprintf(stderr, "SSLConnection: ssl connect fails.\n");
        ERR_print_errors_fp(stderr);
        exit(EXIT_FAILURE);
    } 
...
pair<int, SSL*> SSLConnection::ListenSSL() {
    ...
    if (socketFd < 0) {
        fprintf(stderr, "SSLConnection: socket listen fails: %s.\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
    ...
    if (SSL_accept(sslConnectionPtr) != 1) {
        fprintf(stderr, "SSLConnection: accept the connection fails.\n");
        ERR_print_errors_fp(stderr);
        exit(EXIT_FAILURE);
    }
...
bool SSLConnection::SendData(SSL* connection, uint8_t* data, uint32_t dataSize) {
    ...
    if (writeStatus <= 0) {
        fprintf(stderr, "SSLConnection: write the data fails. ret: %d\n", SSL_get_error(connection, writeStatus));
        ERR_print_errors_fp(stderr);
        return false;
    }
 ...
 */
void SSLConnection::GetClientIp(string& ip, SSL* clientConnection) {
    int clientFd = SSL_get_fd(clientConnection);
    struct sockaddr_in clientAddr;
    socklen_t clientAddrLen = sizeof(clientAddr);
