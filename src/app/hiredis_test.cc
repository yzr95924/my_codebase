...
int main(int argc, char* argv[]) {
    RocksdbDatabase* db = new RocksdbDatabase("tmp");
    ...
    string value_str;
    if (db->QueryBuffer((char*)key, 16, value_str)) {
        cout << "exist" << endl;
        tool::PrintBinaryArray((uint8_t*)value_str.c_str(), 16);
    } else {
        db->InsertBothBuffer((char*)key, 16, (char*)value, 16);
        cout << "not exist" << endl;
    }
    delete db;
    ...
    // redis_reply = (redisReply*)redisCommand(redis_ctx, "GET %b",
    //     key, 16);
    // cout << "Get after delete" << endl;
    // if (redis_reply == NULL) {
    //     cout << "the req key not exist" << endl;
    // } else {
    //     cout << "the req key exist" << endl;
    //     cout << redis_reply->type << endl;
    // }
    // freeReplyObject(redis_reply);
